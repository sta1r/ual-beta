	...
	var Carousel = {
		...
		eachMoveUpdate : function() {
			...
			var maxImgHeight = Math.max.apply(null, $(".owl-carousel img.lazyOwl").map(function() {
				console.log("the largest image height in the carousel is:" + $(this).height() );
				return $(this).height();
			}));
			// calculate slider controls position from top of slider container
			var distFromTop = ( ((maxImgHeight / 2) - 16) + 12);  // 16px is half height of navcontrol buttons , 12px is top margin (these will need to be amended if the sizes change)
			// set position of slider nav buttons from top 
			$(".owl-prev, .owl-next").css("top", distFromTop);
			console.log("nav buttons have been re-positioned.");
		...
		lazyPreload : function($item,$lazyImg){
			...
			function showImage(){
				...
				base.options.lazyEffect === "fade" ? $lazyImg.fadeIn(200) : $lazyImg.show();
			}
		},
