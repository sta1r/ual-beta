...
module.exports = function(grunt) {
    ...
    // Watch tasks for compass and jekyll
    watch: {
        sass: {
        ...
        files: [
            '<%= globalConfig.src  %>/assets/js/{,*/}*.js',
            '<%= globalConfig.src  %>/{,*/}*.html',
            '<%= globalConfig.src  %>/assets/css/*.css',
            '<%= globalConfig.src  %>/assets/img/{,*/}*.{png,jpg,jpeg,gif,webp,svg}'
            ]
        }
    },

    // Livereload with a server
    connect: {
        options: {
            port: 9000,
  ...
  grunt.registerTask('foo', [ 'concurrent:local',
                                'connect:dev',
                                'open'
                              ]);
