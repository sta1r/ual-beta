...
if ($shortEnv == TRUE) { 
  // SEt variable for college id [camb, chelsea, lcf, csm, wimb, lcc]
  // 
  $collegeID = "<t4 type="navigation" id="149"/>";
  $courseID = "<t4 type="content" name="Course ID" output="normal" modifiers=""  />";

  $cambDuplicates = array('LETTERBmgY', 'ALTERNl7oC','IN-FOCFM17','IN-FOC70P5','IN-FOCH93e','INTROD5uQo','WELDIN2uH9','PREFOU7KZp','PRE-FO8G7m', 'INTRODGdVi');
  $wimbDuplicates = array('IN-FOCqB60', 'AFTEREZkYu', 'THEATRZBA4', 'PHOTOGRkeL', 'DUALCIbMxn');

  $internationalDuplicates = array('INTERNu4nN');

  // check whether courseID is in the array
  // 
  // 
  
  
  if (in_array($courseID, $cambDuplicates) && $collegeID == "chelsea" ) {
    $collegeID = "camb";
    
  }

  if (in_array($courseID, $wimbDuplicates) && $collegeID == "chelsea" ) {
      $collegeID = "wimb";
      
    }
  
  if (in_array($courseID, $internationalDuplicates) && (($collegeID == "wimb") || ($collegeID == "chelsea")  )) {
        $collegeID = "chelsea";
      }
$test = new ShortCourse($courseID,$collegeID);
$r = $test->returnXml();
$title = $test->title();
$t = $test->dates();
...
// Logic for checking if dual city course or not

// 1. check course ID 
// echo $test->getCourseID();

// 2. create array of dual city courses 
$dualcitys=array("LONDONnLiQ", "LONDONojef", "LONDONrIrg", "LONDON2tiT", "LONDONHmk9", "LONDONc3xS", "LONDONK9cN", "ARCHITsUOe", "LONDONbSb1", "LONDONcWkZ", "LONDONjlB4", "LONDONgTgy", "LONDON7wBG", "LONDONJiY8", "LONDONbtei", "LONDONGAPX", "NEWYORPzJ7", "NEWYORES16", "NEWYORzVSu", "FASHIO0vwR", "EXPERI28nF", "VISUALH1Sp", "DUALCIkIH9", "DUALCIiu3F", "DUALCIbMxn");

// 3. checking to see if course id is in the array
// $isDualCity will be set to true if course id is in the $dualcitys array
$isDualCity = in_array($test->getCourseID(), $dualcitys);
...
<?php 
	if ($isDualCity) { ?>
		...
		<p><a class="button-link" onclick="addToBasket(<?php echo $date["coursedateid"];?>, '<?php echo addslashes($title);?>', '<?php echo $date["startdate"];?> - <?php echo $date["enddate"];?>', '<?php echo $date["starttime"];?> - <?php echo $date["endtime"];?>', '<?php echo round($date["cost"],0);?>', '<?php echo addslashes($venuename[0]['name']); ?>');return false;" href="#">Add to Basket</a> <strong>Please note that by clicking 'Add to Basket' you are adding all of the above dates to your basket.</strong></p>
...
<div id="basketmessage">
	<div id="close" style="text-align:right;">
		<a onclick="hideBasketMessage();return false;" href="#">x</a>
	</div>
	<h2>The following course has been added to your basket</h2>
		<p>[Course Name]

			<?php if (!$isDualCity) { ?>

			<br />[Course Date], [Course Time]<br />[Course Venue], £[Course Cost]</p>

			<?php } ?>

		<p>Your place is not confirmed until you’ve completed your booking</p>
		<br /><br /><a onclick="hideBasketMessage();return false;" href="#">Add another course</a><div style="display: inline; margin-left: 30px; margin-right: 30px;">&nbsp;</div><a onclick="openBasket();return false;" href="#">Book now</a>
</div>
